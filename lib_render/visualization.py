...
def o3d_mesh(mesh=None, color: list = None, colors: list = None, last_update=None):
    _mesh = last_update
    if _mesh is None:
    ...
    if mesh is not None:
        ...
        if colors is not None:
            _mesh.colors = o3d.utility.Vector3dVector(colors)
...
class StreamVisualization(O3DStreamPlot):
    updater_dict = {}
    save_path = ''

    def __init__(self, *args, **kwargs) -> None:
        StreamVisualization.save_path = kwargs.pop("save_path", "./")
        super().__init__(width=1600, *args, **kwargs)

        StreamVisualization.updater_dict = self.updater_dict
        self.idx = 0

        def save(v):
            save_path = StreamVisualization.save_path

            o3d.io.write_triangle_mesh(os.path.join(save_path, "radar_pcl{}.ply".format(
                self.idx)), StreamVisualization.updater_dict["radar_mesh"].update_item)
            o3d.io.write_triangle_mesh(os.path.join(save_path, "depth0_pcl{}.ply".format(
                self.idx)), StreamVisualization.updater_dict["depth0_mesh"].update_item)
            o3d.io.write_triangle_mesh(os.path.join(save_path, "depth1_pcl{}.ply".format(
                self.idx)), StreamVisualization.updater_dict["depth1_mesh"].update_item)
            o3d.io.write_triangle_mesh(os.path.join(save_path, "pred_smpl{}.ply".format(
                self.idx)), StreamVisualization.updater_dict["pred_smpl"].update_item)
            o3d.io.write_triangle_mesh(os.path.join(save_path, "label_smpl{}.ply".format(
                self.idx)), StreamVisualization.updater_dict["label_smpl"].update_item)
            self.idx += 1
...
SMPL_JOINT_LINES = np.asarray([
    ...
    (9, 12),   # Spine 2 -> Neck
    (12, 13),  # Neck -> Left Shoulder
    (12, 14),  # Neck -> Right Shoulder
    (12, 15),  # Neck -> Head
    (13, 16),  # Left Shoulder -> Left Elbow
    (14, 17),  # Right Shoulder -> Right Elbow
    (16, 18),  # Left Elbow -> Left Wrist
    (17, 19),  # Right Elbow -> Right Wrist
    (18, 20),  # Left Wrist -> Left Hand
    (19, 21),  # Right Wrist -> Right Hand
    (3, 22),   # Spine -> Left Collar
    (3, 23)    # Spine -> Right Collar
])
